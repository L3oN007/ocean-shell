...
import {ASPECT_PHONE, BASE_PRICE} from "@/config/products"
...
import {ISaveConfig, saveConfig as _saveConfig} from "./action"
...
export default function DesignConfigurator({
    ...
    const {mutateAsync: saveConfig, isPending} = useMutation({
        ...
        mutationFn: async (args: ISaveConfig) => {
    ...
    const [width, setWidth] = useState<number>(1242)
    const [height, setHeight] = useState<number>(2688)
    ...
    const changeModel = (model: { label: string, value: string }) => {
        const
            rs = ASPECT_PHONE.find((ip: {
                name: string,
                height: number,
                width: number
            }) => ip.name === model.value);
    ...
    return (
        <div className="relative mb-20 mt-20 grid grid-cols-1 pb-20 lg:grid-cols-3">
            ...
            >
                <div className={ `pointer-events-none relative aspect-[${width}/${height}] w-60 bg-opacity-50 `}>
                    <AspectRatio
                        ref={phoneCaseRef}
                        ratio={width / height}
                        className={`pointer-events-none relative z-50 aspect-[${width}/${height}]`}
                    >
                        <NextImage
                    ...
                    <div
                        className="absolute inset-0 bottom-px left-[3px] right-[3px] top-px z-40 rounded-[32px] shadow-[0_0_0_99999px_rgba(229,231,235,0.6)]"/>
                    <div
                        className={cn(
